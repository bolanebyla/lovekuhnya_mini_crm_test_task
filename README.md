# Мини-CRM

Тестовое задание (Senior Python): многопользовательская мини-CRM

## 🛠 Технологии

- **Python 3.13+**
- **[FastAPI](https://fastapi.tiangolo.com/)** - Фреймворк для создания API
- **[Uvicorn](https://www.uvicorn.org/)** - ASGI-сервер
- **[Dishka](https://github.com/just-work/dishka)** - контейнер внедрения зависимостей
- **[Pydantic](https://docs.pydantic.dev/)** - валидация данных и настройки
- **[SQLAlchemy](https://www.sqlalchemy.org/)** - работа с БД
- **[Alembic](https://alembic.sqlalchemy.org/)** - управление миграциями
- **[Cashews](https://pypi.org/project/cashews/)** - кэширование (для примера кэшируются эндпоинты с аналитикой на 10 секунд)
- **[UV](https://github.com/astral-sh/uv)** - менеджер пакетов Python

## 🏗 Архитектура

Проект построен с использованием следующих архитектурных принципов:

- **Чистая архитектура** - разделение на слои (доменный, прикладной, инфраструктурный)
- **Dependency Injection**
- элементы **Domain-Driven Design** (для упрощения не используются события и агрегаты)

## 📁 Структура проекта

### Слои архитектуры

1. **Доменный слой (Domain Layer)** - содержит бизнес-логику и сущности, не
   зависящие от внешних технологий
2. **Слой приложения (Application Layer)** - содержит use cases, DTO, сервисы,
   интерфейсы репозиториев и т.п.
3. **Инфраструктурный слой (Infrastructure Layer)** - реализация интерфейсов (
   БД, HTTP API, кэш и т.д.)

### Общий подход к организации директорий и файлов
Доменный и прикладной слои объединены в директорию `application` и
организованы по контекстам:
```
mini_crm/
├── application/                 # Доменный и прикладной слои, организованные по контекстам
│   ├── <context_1>/         
│   │   ├── entities/            # Доменные сущности
│   │   ├── interfaces/          # Интерфейсы (репозитории, сервисы)
│   │   │   └── repositories/    # Интерфейсы репозиториев (read и write)
│   │   ├── use_cases/           # Use cases (бизнес-сценарии)
│   │   ├── services/            # Cервисы
│   │   ├── dtos.py              # Data Transfer Objects
│   │   └── errors.py            # Ошибки, специфичные для контекста
│   ├── <context_2>/
│   └── ...
└── infrastructure/             # Инфраструктурный слой
    ├── database/               # Реализация репозиториев (SQLAlchemy)
    ├── http_api/               # HTTP API контроллеры
    ├── cache/                  # Реализация кэширования
    ├── di/                     # Регистрация классов в DI контейнере
    └── ...
```
### Компоненты
- [mini_crm](src/mini_crm) - основная логика приложения
- [commons](src/commons) - утилиты и хелперы (произвольная структура)

## 🚀 Запуск проекта
Каталог `src/` является корневым для python модулей проекта,
например в IDE можно его пометить как sources_root
(в случае запуска из консоли, нужно определить `PYTHONPATH` и сослать на этот
каталог)

### Сборка и запуск docker-compose

В директории [docker-compose](../docker-compose) находится
файл [docker-compose.local.yaml](../docker-compose/docker-compose.local.yaml)
в котором указаны все необходимые сервисы для локальной разработки.

#### Запуск всех сервисов

```shell
cd docker-compose/
```

```shell
docker-compose -f docker-compose.local.yaml -p mini_crm_infrastructure up -d --build 
```

### Установка зависимостей
1. Установите зависимости с помощью uv:

```bash
uv pip install .
```

2. Настройте переменные окружения:

```
# Пример конфигурации
DB_URL=postgresql+asyncpg://mini_crm:mini_crm@localhost:5432/mini_crm
LOGGING_LEVEL=INFO
LOGGING_JSON=False
SECRET_KEY=strong_secret_key
```

### 📃 Миграции

Применение миграций

```shell
python -m mini_crm.run.alembic_runner upgrade head
```

`alembic_runner` это обертка, аргументы просто проксируются в cli alembic

```shell
python -m mini_crm.run.alembic_runner <other args>
```

### HTTP API

Запуск сервера для разработки http api

```bash
uvicorn mini_crm.run.http_api:app --reload
```

### Docker

Сборка образа

```shell
docker build -t lovekuhnya_mini_crm_test_task .
```

Применение миграций

```shell
docker run mini_crm:latest entrypoint_migrations.sh
```

Запуск http api

```shell
docker run -p 8080:8080 mini_crm:latest entrypoint_http_api.sh
```

## 🧪 Тестирование

Запуск тестов

```bash
pytest
```

## Openapi документация

- Файл документации доступен по адресу http://127.0.0.1:8000/openapi.json
- Swagger доступен по адресу http://127.0.0.1:8000/docs

## 📊 Мониторинг

Метрики доступны по адресу `/metrics` (Prometheus)


## 🔍 Линтеры

В проекте используется [Ruff](https://github.com/astral-sh/ruff)

### Настройка линтера

Конфигурация линтера находится в файле `pyproject.toml` в секции `[tool.ruff]`

### Запуск линтера

```bash
ruff check .
```

Автоматическое исправление проблем

```bash
ruff check --fix .
```

## ✅ Проверка типов

Для проверки типов используется MyPy

Запуск MyPy

```shell
mypy .
```


## 🔒 Pre-commit

В проекте настроен pre-commit для автоматической проверки кода перед коммитом.
Настройки pre-commit находятся в файле `.pre-commit-config.yaml`.

### Установка pre-commit

```bash
pre-commit install
```

После этого pre-commit будет автоматически запускаться перед каждым коммитом.